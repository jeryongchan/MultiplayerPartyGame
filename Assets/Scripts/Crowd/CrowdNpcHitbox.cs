...
namespace FriendSlop.Crowd
{
    ...
    // a player hit (NetworkObject in parent) or world/wall (neither). lives on the NPC root (above the
    // per-bone Hitbox_* colliders), so the shooter's GetComponentInParent finds it from any bone struck.
    //
